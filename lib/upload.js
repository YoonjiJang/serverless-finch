 ...
 */
function uploadDirectory(aws, bucketName, clientRoot, headerSpec, orderSpec, basePath) {
  ...
  return filesGroupedByOrder.reduce((existingUploads, files) => {
    return existingUploads.then(existingResults => {
      ...
      return BbPromise.all(
        uploadList.map(u => uploadFile(aws, bucketName, u.filePath, u.fileKey, u.headers, basePath))
 ...
 * @param {string} clientRoot - Full path to the root directory of client files
 */
function uploadFile(aws, bucketName, filePath, fileKey, headers, basePath) {
  let baseHeaderKeys = [
    'Cache-Control',
    'Content-Disposition',
    'Content-Encoding',
  ...
  let params = {
    Bucket: bucketName,
    Key: path.join(basePath || '', fileKey),
    Body: fileBuffer,
