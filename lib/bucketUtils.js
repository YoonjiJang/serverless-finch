 ...
 * @param {Object} aws - AWS class
 * @param {string} bucketName - Name of bucket to be deleted
 * @param {string} basePath - Name of base path to be deleted
 */
function emptyBucket(aws, bucketName, basePath) {
  return listObjectsInBucket(aws, bucketName).then(resp => {
    ...
    if (!contents[0]) {
      ...
      const objects = _.map(_.filter(contents, function(content) {
        const fileKey = path.join('/', content.Key);
        const base = path.join('/', basePath || '', '/');
        return fileKey.startsWith(base);
      }), function(content) {
